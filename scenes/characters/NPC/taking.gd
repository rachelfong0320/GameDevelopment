...
func _on_enter() -> void:
	if npc.npc_direction == Vector2.UP:
		...
		hit_component_collision_shape.position=Vector2(-1,-10);
	elif npc.npc_direction == Vector2.RIGHT:
		animated_sprite_2d.play("take_right")
		hit_component_collision_shape.position=Vector2(15,0);
	elif npc.npc_direction == Vector2.DOWN:
		animated_sprite_2d.play("take_front")
		hit_component_collision_shape.position=Vector2(1,24);
